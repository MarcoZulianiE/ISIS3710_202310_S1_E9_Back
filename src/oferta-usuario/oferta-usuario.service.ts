...
export class OfertaUsuarioService {
  ...
  async addUsuarioOferta(
    ofertaId: string,
    usuarioId: string,
  ): Promise<OfertaEntity> {
    const usuario: UsuarioEntity = await this.usuarioRepository.findOne({
      where: { id: usuarioId },
    });
    if (!usuario)
      throw new BusinessLogicException(
        NotFoundErrorMessage('usuario'),
        BusinessError.NOT_FOUND,
      );

    const oferta: OfertaEntity = await this.ofertaRepository.findOne({
      where: { id: ofertaId },
      relations: ['contrato', 'horarios', 'usuario'],
    });
    if (!oferta)
      throw new BusinessLogicException(
        NotFoundErrorMessage('oferta'),
        BusinessError.NOT_FOUND,
      );
  ...
  async findUsuarioByOfertaIdUsuarioId(
    ofertaId: string,
    usuarioId: string,
  ): Promise<UsuarioEntity> {
    const usuario: UsuarioEntity = await this.usuarioRepository.findOne({
      where: { id: usuarioId },
      relations: ['necesidades', 'especialidades'],
    });
    if (!usuario)
      throw new BusinessLogicException(
        NotFoundErrorMessage('usuario'),
    ...
    const ofertaUsuario: UsuarioEntity =
      oferta.usuario.id === usuario.id ? oferta.usuario : null;

    if (!ofertaUsuario)
      throw new BusinessLogicException(
        PreconditionFailedErrorMessage('oferta', 'usuario'),
  ...
  async associateUsuarioOferta(
    ...
    const usuarioEntity: UsuarioEntity = await this.usuarioRepository.findOne({
      where: { id: usuario.id },
    });
    if (!usuarioEntity)
      throw new BusinessLogicException(
